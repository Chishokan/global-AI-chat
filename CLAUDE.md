...
- **Phase 1（実装済み）**: 多言語Q&Aチャット（KBグラウンディング／7言語の言語選択＋やさしい日本語／地域切替 佐世保・西海でKB切替／5タブUI チャット・FAQ・こまったとき・てつづき・設定／スタッフ誘導の座席）。DBは使わず /data/faq.json を直接KBソースとして利用。会話ログ（スプレッドシート・任意）あり。
